			...
			local function onMouseEvent( event )
				...
				if event.type == "down" and clickjournal.status == 'yes' then
					...
					function rect_gen()
						...
						------ This function warps the generated shape from one side to the other.
						local myListener = function( event )
								if rect1.x >= 515 then rect1.x = -42
								elseif rect2.x >= 515 then rect2.x = -42
								elseif rect1.y <= 1 then rect1.y = 313
								elseif rect2.y <= 1 then rect2.y = 313
								else
								end
						end
					...
					function roundrect_gen()
						...
						---- This function warps the generated shape from one side to the other.
						local myListener2 = function( event )
								if myRoundedRect.x >= 515 then myRoundedRect.x = -42
								elseif myRoundedRect.x >= 515 then myRoundedRect.x = -42
								elseif myRoundedRect.y <= 1 then myRoundedRect.y = 313
								elseif myRoundedRect.y <= 1 then myRoundedRect.y = 313
								else
								end
						end
						Runtime:addEventListener( "enterFrame", myListener2 )
